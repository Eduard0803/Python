...
if email == "teste@gmail.com":
    if senha == "password":
        ...
        ):  # Função que gera os códigos de autenticação
        ...
        )  # Parte respónsavel por fazer a ligação ao usuario
        ...
        ligacao = cliente.calls.create(
            to=meu_numero, from_=numero_twilio, twiml=mensagem
        )

        aut = input(
            "Digite o código que recebeu na chamada telefonica: "
        )  # Pede a entrada do código ao usuario

        if (
            aut == texto
        ):  # Faz a comparação entre o código de autenticação gerado e o código inserido pelo usuario
